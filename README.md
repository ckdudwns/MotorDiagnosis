...
> 두 모델 연동용 신규 규격: **`edge-feature-history-v1` · 25초 등간격 이력(이상 중에도 유지)**.
> [서버 탑재·IoT 전달 규격](docs/pump-dual-model-serving.md). 기존 `edge-feature-snapshot-v1` 수신은 보존하지만 새 모델 이력에 섞지 않습니다. 코드와 실제 서버·보드 배포는 구분합니다.
>
...
기존 RF66의 **서버 기동 시 모델 로딩·자동 추론·자동 이벤트 투영은 중지**했습니다.
과거 결과와 원본은 보존하며, 이전 모델 설정이 남아 있어도 RF66을 자동 재연결하지 않습니다.
새 단건 입력은 [입력 준비·모델 대기 경로](docs/periodic-single-snapshots.md)로 처리합니다.
기존 [특징값 전용 규격](docs/edge-feature-snapshots.md)은 호환 수신 경로로 유지합니다. 요청은 Raw 없이
평균 제거한 XYZ의 CF·왜도·Pearson 첨도 9개만 저장하며, invalid는 null과 원인을 보존합니다.
이전 규격의 정기 간격은 **25·25·10초**이고 이상 상태에서는 정기를 중단합니다. 이 입력에 이력 모델을 자동 적용하지 않습니다.
`pump-event-verifier-v1` JSON 모델은 **센서별 25초 특징 9개와 직전 이력 24건**을 사용합니다.
이전 `pump-verifier-history-v1` 호환 경로는 [펌프 이력 검증 문서](docs/pump-event-verifier.md)를 참고합니다.
모델 규격과 호환되지 않는 입력은 저장·준비하되 `MODEL_INPUT_CONTRACT_MISMATCH`로 판정 불가 처리합니다.
모델 파일 해시·학습 stream·실제 sensorId를 설정해야 하며, 미설정 센서는 모델 대기 상태입니다.
이상 확인 점수는 고장 확률이 아닙니다.
현재는 [두 모델 연동](docs/pump-dual-model-serving.md)을 추가했습니다. 새 프로파일
`adxl345-ac-cf-sk-ku-25s-v1`은 25초 등간격 이력과 최신 window 선택 규격을 명시합니다.
...
**IoT 상태 기반 단건 수신·입력 준비·추론 연결부(3단계):** `POST /api/devices/{deviceId}/periodic-snapshots`는
...
별도 처리 작업이 샘플·해시를 다시 검증하고 `[512, 3] XYZ g` 입력을 준비해 `waiting_model`로 저장합니다.
교체 모델은 아직 없으므로 기본 실행에서는 실제 추론·이벤트·알림을 적용하지 않습니다.
3단계에는 명시적으로 전달한 단건 모델 어댑터의 입력 계약·대상 장치·버전·임계값 검증,
별도 추론 작업 및 결과 저장을 추가했습니다. 기존 RF66 특징/모델을 대체품으로 사용하지 않습니다.
모델 연결 전 자료와 완료 결과는 자동 재판정하지 않으며, 향후 모델 파일 검증·전처리 구현은 별도 작업입니다.
기존 `periodic-single-v1` 요청은 호환 유지하며, 기존 RF66 경로는 원본 수신·과거 조회만 남기고 자동 분석을 끕니다.
입력 필드·4종 전송 예시·재전송·검증 절차는 [IoT 단건 수신 규격](docs/periodic-single-snapshots.md)을 참고하세요.
교체 모델 담당자와 맞출 연결 계약은 [단건 모델 어댑터 규격](docs/snapshot-model-adapter.md)을 참고하세요.

**단건 사건·알림 연결:** 서버의 유효한 이상 판정 1건으로 사건을 열고, 같은 장치·설비·모델의 정상 판정 1건으로 닫습니다.
보드의 3회/5회 카운터나 기존 RF66 연속 3구간 조건을 서버 판정에 재사용하지 않습니다.
품질 불량·미수신·모델 변경은 관측 불명으로 유지하며 자동 복귀하지 않습니다.
`SNAPSHOT_EVENT_MODE=events`가 기본(사건 기록, 알림 꺼짐)이며 `shadow`는 비교만,
`alerts`는 기존 알림 정책·채널·수신자 설정 및 최신 입력 검사를 통과한 발생/복귀에 한해 알림을 허용합니다.
실제 모델이 없는 현재 앱에서는 어느 모드에서도 새 모델 사건·알림을 생성하지 않습니다.
상세 정책·재시작·운영 설정은 [단건 사건·알림 운영](docs/snapshot-events.md)을 참고하세요.

웹에서는 **운영 현황 → 단건 진동 수신 · 새 모델 판정**에서 최근 측정 최대 20건을 확인합니다.
측정·수신 시각, 보드 상태·전송 사유, 품질, 모델 대기·추론·완료 상태와 결과별 점수·모델을 구분합니다.
운영 현황에서 5초마다 조회하며, 기존 조회 기간 필터·RF66 과거 이력·통계 점수와는 별개입니다.
모델이 없는 현재 기본 실행에서는 `모델 대기`로 표시하며 정상 판정이나 알림을 만들어내지 않습니다.
자세한 표시 기준은 [단건 웹 화면](docs/periodic-single-snapshots.md#웹-화면)을 참고하세요.
