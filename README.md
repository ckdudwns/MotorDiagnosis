...
> 신규 보드 연동 기준: **특징값 전용 `edge-feature-snapshot-v1`**. 정상 UTC 00·25·50초, 이상 진입/복귀 즉시, 유지 10초. [서버·IoT 규격](docs/edge-feature-snapshots.md). 배포 여부는 별도 확인.
>
...
최신 기획안은 [특징값 전용 규격](docs/edge-feature-snapshots.md)을 우선합니다. 새 요청은 Raw 없이
평균 제거한 XYZ의 CF·왜도·Pearson 첨도 9개만 저장하며, invalid는 null과 원인을 보존합니다.
정기 간격은 **25·25·10초**이고 이상 상태에서는 정기를 중단합니다. 이 입력에 아래 기존 이력 모델을 자동 적용하지 않습니다.
새 `pump-event-verifier-v1` JSON 모델은 **센서별 25초 특징 9개와 직전 이력 24건**을 사용하는
[펌프 이력 검증 경로](docs/pump-event-verifier.md)에 명시적으로 연결할 수 있습니다.
아래 모델은 이전 `pump-verifier-history-v1` 호환 경로용입니다. 정확한 25초 이력이 필요한 규격이므로 최신 특징값 전용 규격과 다릅니다.
호환되지 않는 새 입력은 저장·준비하되 `MODEL_INPUT_CONTRACT_MISMATCH`로 판정 불가 처리합니다.
모델 파일 해시·학습 stream·실제 sensorId를 설정해야 하며, 미설정 센서는 모델 대기 상태입니다.
이상 확인 점수는 고장 확률이 아니며 별도의 5분 뒤 예측 모델은 아직 연결하지 않았습니다.
