...
담수급수 모터의 센서별 엑셀 요약 특징은 [양수장 학습 실험 안내](docs/pump-summary-experiment.md)를
따릅니다. 이 경로는 단위 미확정 데이터의 오프라인 비교 실험이며 기존 운영 모델을 교체하지 않습니다.
현재는 최근 12개 연속 측정으로 다음 5초의 특징을 예측하는 학습·평가 코드까지 제공하며, 최신 측정
한 건을 받는 실시간 서버 추론·대시보드 그래프는 아직 연결되어 있지 않습니다.
