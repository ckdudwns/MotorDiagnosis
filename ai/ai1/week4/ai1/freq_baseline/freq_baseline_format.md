...
| LSTM Autoencoder | 연속 윈도우 시퀀스(길이 5) | 원신호를 매니페스트와 같은 윈도우(2048)로 재분할해 `features.py`로 벡터화 | 한 원본 파일의 모든 시퀀스는 그 파일이 배정된 한 split에만 들어간다 (파일 내부 재분할 없음) |

예전 구현은 Dense가 `split` 컬럼만 읽고 원본을 다시 잘라 특징을 재계산했고(전처리
설정이 어긋나면 `datasetId`가 거짓말), LSTM은 파일 내부 청크를 다시 70/20/10으로
나눠 같은 원본이 train/val/test에 모두 들어갔다. 둘 다 수정됐다.
