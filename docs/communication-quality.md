...
기본 예제는 품질 채널을 비활성화한다. MQTT 브로커/PUBACK 계측은 포함하지 않는다. 기존 장치 health API의 RSSI/버퍼 사용률 보고는 유지한다. 후속 [장치 운영 화면](core-operations-ui.md)에서 이 API의 기간별 품질과 결측 상태를 조회할 수 있다.
...
품질 창을 한 번이라도 저장한 장치는 삭제·동일 ID 재등록을 `409 DEVICE_HAS_COMMUNICATION_QUALITY_HISTORY`로 거부한다. 시도/ACK가 0인 유휴 창도 이력에 포함한다. 교체할 때는 기존 장치를 비활성화하고 새 장치 ID를 사용한다. 원시 창이 30일 후 만료돼도 장치 ID만 담은 보호 기록은 유지한다. 일반 상태가 구버전으로 복원되거나 이전 버전에서 장치를 삭제한 경우에도 품질 DB에 남은 ID는 재등록할 수 없다. 과거에 이미 재사용된 ID를 자동으로 분리·수정하지는 않는다.

품질 저장소 스키마 v2는 기존 v1 창의 장치 ID를 **시작 시 만료 정리 전에** 보호 기록으로 옮긴다. 이후 창과 보호 기록을 같은 transaction에서 저장하며, 저장 실패 시 둘 다 되돌린다. 삭제·등록·매핑 변경과 품질 저장은 같은 수명주기 잠금으로 조정한다. 이력 조회 실패 시 삭제·재등록은 503으로 거부하고 장치 상태를 바꾸지 않는다. 이력 DB 대기는 일반 텔레메트리 상태 잠금 밖에서 수행한다. wire `schemaVersion`과 펌웨어는 계속 v1이다. 스키마 v2 DB는 구버전 서버에서 열 수 없으므로 롤백 시 v2 호환 서버를 사용하고, 보호 테이블이나 버전 값을 강제로 삭제·낮추지 않는다.
...
- `tests/test_communication_quality.py`: 인증·범위·입력 경계, 중복/충돌/겹침, 동시 제출, 가중 평균/빈 구간, 장시간 창, 저장 실패/재시작, 보관기간과 실제 HTTP API. 품질만 있는 장치의 삭제·재등록 차단, 구버전 이력 이관, 만료 후 ID 보호, 이력 DB 실패, 삭제/저장 양방향 경합도 검증한다.
