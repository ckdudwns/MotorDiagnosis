...
추가 구현 목록 IOT-01/02에 대응한다. 펌웨어 버전은 `v1.3-iot-health.2`이며,
...
### 과거 텔레메트리와 health 전송 순서 (PR23 보완)

오류/회복 전이는 백엔드의 분석 경계를 이동시키므로 과거 측정값보다 먼저 보내지 않는다.
매 전송에서 링의 가장 오래된 측정 시각과 health FIFO의 다음 전이 시각을 비교한다.
측정 시각이 경계보다 이전이거나 같으면 텔레메트리를 먼저 ACK까지 처리하고,
이후 측정이면 전이 확인 응답과 NVS 제거가 완료될 때까지 해당 측정을 보류한다.
한 배치의 호출 순서만 바꾸는 것이 아니라 모든 head와 후속 배치에서 다시 판정한다.

오류 전이를 기다리는 동안에도 RSSI 등은 `sensorFaults:[]`인 메트릭 전용 요청으로
갱신한다. 현재 백엔드에서 빈 배열은 기존 센서 고장을 해제하지 않는다. 이 응답으로
대기 중 전이를 제거하지 않으며, backlog가 있으면 주기 상태 snapshot도 보내지 않는다.
따라서 센서 오류 알림은 이전 데이터의 재전송 완료까지 지연될 수 있지만, 현재 RSSI 등
상태 수치의 보고는 유지한다. 분석 순서 보존과 즉시 오류 알림을 동시에 제공하려면
백엔드의 과거 데이터 분석/알림 계약을 별도로 확장해야 한다.
head를 읽지 못하거나 UTC가 복원되지 않았다면 오류 전이를 앞질러 보내지 않고,
기존 링의 재시도/격리 규칙을 따른다. 확인 시각이 없는 이전 부팅의 전이는 기존
수신 시각 fallback을 유지하며, 그 시각보다 오래된 backlog가 먼저 처리된다.

새 텔레메트리도 미전송 전이가 있으면 직접 POST하지 않고 링에 넣어 같은 판정을 받는다.
텔레메트리는 초, health는 밀리초 단위이므로, 확인된 경계와 같은 초에 새 측정이
생성되지 않도록 다음 초까지 수집 시작을 짧게 보류한다. 발생 시각이나 측정값을 바꾸지 않는다.
이 순서 계약은 백엔드에 이미 ACK한 데이터의 분석 오류를 소급 복구하는 기능은 아니다.
