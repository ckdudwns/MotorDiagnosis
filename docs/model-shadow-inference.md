...
`SHADOW_MODEL_ARTIFACT`를 해제해도 **같은 `SHADOW_MODEL_DB_PATH`를 유지**한다.
서버는 추론 실행과 별개로 `ModelHistoryGuard`를 등록해 기존 모델 DB의 영구 `identities`를 읽는다.
모델 파일·PyTorch·NumPy가 없어도 이력이 있는 장치의 삭제와 이미 삭제된 ID 재등록은 `409 DEVICE_HAS_HISTORY`로 차단한다.
입력/결과 보관기간이 지나 식별자만 남아도 보호는 유지된다.

비활성 상태에서는 추론 작업자를 시작하거나 대기 입력을 계산하지 않는다. 보호용 조회는 읽기 전용이며 모델 DB를 생성·마이그레이션하거나 기존 입력/결과를 바꾸지 않는다.
매 삭제/등록 시 DB를 확인하므로 서버 시작 이후 추가된 이력도 반영한다. 기존 DB의 읽기 오류·스키마 손상·확인했던 파일의 유실은 `503 MODEL_HISTORY_UNAVAILABLE`로 처리하고 식별자 변경을 거부한다.
아직 모델 DB를 한 번도 만들지 않은 신규 환경에서는 이력 없는 장치 작업을 허용한다. `:memory:` 저장소는 재시작 보존 대상이 아니며 실행 중인 저장소의 기존 보호만 적용한다.

장치 이력을 숨기기 위해 DB 경로를 바꾸거나 파일을 삭제하지 않는다. 서버 종료/초기화 실패 시 보호 등록은 정리된다.
관련 검사는 `python -m unittest tests.test_model_history_guard -q`이며 ML 의존성과 전달 모델 ZIP 없이 실행할 수 있다.
2026-09-07 재검증: 전용 11개 통과, 전체 백엔드 428개 중 426개 통과·선택 Paho 2개 제외, 화면 88개 통과. 전체 검사에는 실제 전달 모델/원시 파형 연계 검사도 포함된다.
