# IoT 상태 기반 단건 수신·입력 준비·추론·웹 조회·사건 연결

## 범위와 현재 상태

> 최신 보드 연동은 [특징값 전용 규격](edge-feature-snapshots.md)을 우선합니다. Raw 없이 UTC 00·25·50초, 이상 유지 10초, 진입/복귀 즉시 전송합니다.
> 아래는 **기존 Raw 5분/10초 단건 정책**의 호환 경로입니다. [이전 펌프 이력 검증 계약](pump-event-verifier.md)도 새 특징 프로파일과 별개입니다.

- 이 문서의 기존 계약은 **`edge-state-snapshot-v1`**입니다. 보드는 모든 측정 구간을 판단하되 아래 네 조건에서만 현재 Raw 하나를 선택합니다.
...
- `quality=valid`는 우선 `queued`로 저장합니다. 모델 없는 기본 실행에서는 입력 준비 뒤 `waiting_model`로 바뀌며 점수를 만들지 않습니다.
- 3단계 공통 어댑터 외에 새 JSON 이력 모델 로더를 추가했습니다. 설정이 없으면 모델을 공급하지 않으며, 기존 Raw를 새 모델 특징으로 임의 변환하지 않습니다.
...
- 웹 조회와 [새 경로의 사건·알림](snapshot-events.md)을 연결했습니다. 펌웨어 변경·모델 적용 대상 선택·운영 배포는 별도입니다.
...
로그인 후 **운영 현황 → 사이트·설비 선택 → 단건 진동 수신 · 새 모델 판정**에서 조회합니다.
기존 RF66 과거 이력 패널 바로 위에 표시하며, 기존 통계 점수·장치 health와 별개입니다.

- 인증된 장치 목록과 `GET /api/devices/{deviceId}/periodic-snapshots`를 직접 조회합니다.
  권한은 API와 동일하게 `device:read`, `telemetry:read`이며 별도 `model:read`를 요구하지 않습니다.
- 운영 현황에서 5초마다 갱신하고 새로고침 버튼도 사용합니다. 장치 전송을 요청하거나 주기를 바꾸는 동작은 아닙니다.
  기존 통계·RF66 API 실패가 새 단건 조회를 막지 않습니다. 느린 동일 범위 조회는 중복 실행하지 않습니다.
  개별 조회가 15초를 넘으면 취소·실패 표시 후 다음 갱신에서 재시도합니다.
- 조회 기간 필터와 별개인 **측정 시각 기준 최근 최대 20건**입니다. 전체 저장 이력·기간 통계·내보내기가 아닙니다.
- 측정 시각, 해당 구간 최초 서버 수신 시각, 보드 상태·전송 사유·연속 횟수, 품질·불가 사유,
  처리 대기·모델 대기·추론 중·판정 불가·완료 판정을 구분합니다. 보드 NORMAL/ANOMALY_ACTIVE는 서버 모델 판정이 아닙니다.
- 완료 결과는 점수, 임계값 및 비교 방향, 점수 종류, 적용 모델·전처리 버전, 판정 완료 시각을 표시합니다.
  점수를 일률적으로 0~1 확률로 해석하지 않습니다. 이전 설정의 결과와 최신 입력의 대기 상태도 분리합니다.
- GET의 `queriedAt`은 서버 조회 UTC 시각입니다. 화면의 최신성은 이 시각과 **최신 측정 시각**을 대조합니다.
  NORMAL은 300초, ANOMALY_ACTIVE는 10초에 각각 여유 60초를 더하고, 5초 넘는 미래 시각은 확인 필요로 표시합니다.
  즉시 전송의 `intervalSec: 0`을 다음 보고 간격으로 사용하지 않습니다. 과거 큐의 최근 도착을 새 측정으로 간주하지 않습니다.
- 모든 시각은 브라우저 현지 시간대로 표시합니다. 최신성 문구는 장치 온라인/오프라인이나 장비 정상 보장이 아닙니다.
- 조회 실패·권한 부족·빈 입력·모델 미설정을 구분합니다. 선택·로그인·화면 변경 전의 늦은 응답은 표시하지 않습니다.
- 단건 모델 사건은 **이벤트 검수**, 발송된 웹 알림은 **운영 현황 → 웹 알림**에서 확인합니다.
  기본 모드는 `events`(기록만)이며 모델이 설정되지 않은 실행에서는 사건도 생성하지 않습니다.
  단건 패널에는 운영 모드와 구간별 사건 처리 상태를 표시합니다. RF66은 계속 과거 이력 전용입니다.

화면 회귀 테스트: `node --test tests/test_snapshot_dashboard.mjs`.
실제 저장소 응답과 브라우저 코드의 연결 검증: `python -m unittest tests.test_snapshot_dashboard -q` (Node.js 필요).
...
- 기존 DB 스키마 1·2는 3으로 자동 이전합니다. 원본·수신 시각·digest·완료 결과를 변경하지 않고 추론 작업과 사건 작업·사건 저장 테이블을 추가합니다. 이전 자료에 사건 작업을 소급 배정하지 않습니다. 수신 때 이미 허용된 대기 자료가 48시간보다 오래됐다는 이유로 준비 단계에서 버리지 않습니다. 새로 수신하는 오래된 요청에 대한 제한은 그대로입니다.
...
- 기존 온라인 판정·통계 사건·RF66 이력은 변경하지 않습니다. 새 모델 사건은 `source: snapshot`으로 구분합니다.
