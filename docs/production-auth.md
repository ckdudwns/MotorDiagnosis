...
이 문서는 **운영자 로그인 계정**을 다룹니다. 운영 수집 인증은 [운영 수집 토큰 안내](production-ingest-auth.md)를 함께 적용하세요. 수집 인증 보호 변경이 반영된 버전은 `APP_ENV=production`에서 기본 수집 토큰을 차단합니다. 운영용 장치 토큰 발급·설정과 HTTPS 인증서·프록시·CORS 허용 출처·운영 데이터·백업 설정을 확인하기 전에는 수집 API를 인터넷에 공개하지 마세요.
