...
RF66 이벤트 모드 `events`를 적용한 이후의 운영 도구다. 서버 코드 머지·적용 후
...
- 원시 300,000건 / 특징 500,000건 한도의 90% 이상 경고.
- 처리 대기 3,276건 이상 또는 가장 오래된 대기가 30초를 넘으면 경고.
- 장치 원시 데이터 없음·측정 시각 정지·30초 이상 오래된 입력·품질 불량을 구분.
- RF66 이벤트 9,000건 이상, runtime DB에 아직 반영되지 않은 이벤트,
  120초 이상 지난 알림 대기를 확인.
- 정상 수신 여부는 선택한 장치의 원시 데이터로 확인한다. 예전 요약 텔레메트리의
  마지막 수신 시각만으로 원시 입력 정상 여부를 판정하지 않는다.

원시 구간은 48시간 보관하고 이미 접수한 미처리 구간은 자동 삭제하지 않는다.
