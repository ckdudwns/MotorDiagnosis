...
1/3부터 다시 시작합니다. 순번이 이어져도 시작 uptime 간격이 **630~670ms(양 끝 포함)**를
벗어나면 `TIME_GAP`으로 초기화합니다. 하한 630ms는 유지하고 상한만 650ms에서
670ms로 완화했습니다. 새 결과의 `confirmation.startIntervalRangeUs`에
`[630000, 670000]`을 저장하며 이벤트 발생·해제도 동일한 시간 기준을 사용합니다.
이는 소프트웨어 연속성 허용 범위이며
...
2026-09-09 01:45~09:00 KST 측정의 서버 추출 결과에서 유효·연속 번호 265쌍의
중앙값은 655.246ms, 기존 630~650ms 통과는 0쌍이었습니다. 사용자 실행 재계산에서
630~670ms 적용 시 251쌍(94.72%)이 시간 검사를 통과했고, 분석 완료 1,102구간 중
204구간이 3구간 확인 가능해졌습니다(연속 이상 89, 연속 이상 아님 115, 대기 898).
품질 불량 830구간은 그대로 제외됐습니다. 이는 과거 자료의 후처리 비교이며
현장 정확도나 무손실 수집의 입증이 아닙니다. 과거 저장 결과·이벤트는 변경하지
않고, 배포 후 새로 처리하는 구간(queued 포함)에만 새 기준을 사용합니다.
