...
> 현재 서버에서는 RF66 모델 로딩·자동 추론·자동 사건 투영을 중지했습니다.
> 아래는 이전 구현/오프라인 재현용 기록이며, RF66 환경 변수를 설정해도 현재 서버가 모델을 로딩하지 않습니다.
> 새 경로는 [단건 입력 준비·모델 대기](periodic-single-snapshots.md)를 사용합니다. 과거 원본·결과는 보존합니다.
