"""Feature-only ADXL345 contract; not the workbook's 25-second history profile.

Normal reports use UTC seconds-of-minute 00/25/50 (25/25/10 seconds), not a
uniform 25-second grid. Board state is evidence, never a server-model verdict.
"""
import copy
import math
import re
...
def normalize(window, *, check_time_bounds=True):
    ...
    if type(window["schemaVersion"]) is not int or window["schemaVersion"] != 2 or window["profileId"] != PROFILE_ID:
    ...
    if slot is not None:
        ...
        if type(slot) is not int or not 0 <= slot <= 2**53-1 or slot % 60 not in (0, 25, 50):
            invalid("periodicSlotEpoch must identify UTC second 00, 25 or 50")
        width = 10 if slot % 60 == 0 else 25
        if quality == "valid":
...
def validate_delivery(info, window):
    ...
    states = {"periodic": "NORMAL", "anomaly_start": "ANOMALY_ACTIVE",
              "anomaly_active": "ANOMALY_ACTIVE", "recovery": "NORMAL"}
    if not isinstance(event, str) or event not in states or state != states[event]:
        invalid("Invalid eventType/state; periodic is not generated while anomaly is active")
    ...
    if event == "anomaly_active" and slot is not None:
...
def prepare(window):
    ...
    return {**copy.deepcopy(INPUT_CONTRACT), "values": [window["features"][k] for k in FEATURES]}
