    ...
    def test_inspection_uses_periodic_cadence_and_received_processing_age(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertNotIn("RAW_INPUT_STALE_OR_FUTURE",[i["code"] for i in report["issues"]])
                self.assertTrue(report["databases"]["RAW_VIBRATION_WINDOW_DB_PATH"]["historicalOnly"])
                self.assertNotIn("RAW_VIBRATION_WINDOW_DB_PATH:PROCESSING_BACKLOG",[i["code"] for i in report["issues"]])
            finally: store.close()
